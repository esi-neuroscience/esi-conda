#!/bin/bash
#
# Uninstall esi-conda + env in macOS + Linux
#
# This script is loosely based on the Homebrew uninstaller (albeit heavily truncated):
...
config_restore() {
  configFile="$1"
  configBackup=`find "${HOME}" -maxdepth 1 -mindepth 1 -type f -name "${configFile}"'.esi-conda.backup*'`
  if [[ ! -z "${configBackup}" ]]; then
    ...
    execute "awk" '{gsub("conda activate esi-conda", "# conda activate esi-conda"); print}' "${HOME}/${configFile}" >| "${HOME}/.tmp${configFile}"
    execute "mv" "${HOME}/.tmp${configFile}" "${HOME}/${configFile}"
    debug "Removed 'conda activate esi-conda*' from ~/${configFile}"
  fi
}

# (Try to) fetch esi-conda installation directory from config file(s)
get_condabinpath() {
  configFile="$1"
...
# Default esi-conda installation directory
...
if [[ -z "${condaBinPath-}" ]]; then
  error "No esi-conda installation found on this system. Exiting..."
fi

# Check if we're working with a default installation
if [[ "${condaBinPath}" == *"${CondaInstallationDirectory}"* ]]; then
  debug "Found esi-conda in default location"
else
  CondaInstallationDirectory="${condaBinPath%%/bin/conda}"
  warn "Found non-standard esi-conda installation at ${CondaInstallationDirectory}"
  if [[ -z "${ncNoninteractive-}" ]]; then
...
if [[ ! -z "$(command -v conda)" ]]; then
  if [[ -z "$(conda env list | grep esi-)" ]]; then
    if [[ -z "${ncNoninteractive-}" ]]; then
      warn "esi-conda environment NOT found in ${CondaInstallationDirectory}. Are you sure you want to delete this installation?"
      user_input
    else
      warn "Removing ${CondaInstallationDirectory} despite it not containing a esi-conda environment"
    fi
  fi
else
