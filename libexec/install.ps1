...
Else {
    ...
    If (-not $Env:ncNoninteractive) {
        Write-Warning "Do you really want to install esi-conda alongside the existing miniforge3?"
        User-Input
    }
    Else {
        Write-Host "Installing esi-conda alongside existing miniforge3"
    }
}

If (-not (Get-Command "conda" -errorAction SilentlyContinue)) {
    throw "Conda is installed but not available in this PowerShell. Please continue with the esi-conda installation from a PowerShell with conda activated."
}
...
If (-not $Env:ncCI) {
    ...
    Invoke-WebRequest $ESICondaLatestUrl -OutFile "$Env:temp\latest.yml"
    $filename = "$Env:temp\latest.yml"
    Write-Debug "Downloaded $ESICondaLatestUrl to $Env:temp\latest.yml"
}
Else {
    $filename = "envs\x86\latest.yml"
    Write-Debug "Using local repository version of latest environment file"
}

Write-Host "Creating latest esi-conda environment"
mamba env create --file $filename
Write-Debug "Done"

If ($Env:ncEditor) {
    Write-Host "Installing Spyder on Windows is not yet supported. You can add it later on by activating the esi-conda environment and installing it via"
