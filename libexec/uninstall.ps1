# Uninstall esi-conda in Windows
...
try {
    ...
    Write-Error "No esi-conda installation found on this system. Exiting..."
}

# Check if we're working with a default installation
If ($condaBinPath.Contains($CondaInstallationDirectory)) {
    Write-Debug "Found esi-conda in default location"
}
Else {
    $CondaInstallationDirectory = $condaBinPath
    Write-Warning "Found non-standard esi-conda installation at $CondaInstallationDirectory"
...
Else {
    If (-not $Env:ncNoninteractive) {
        Write-Warning "esi-conda environment NOT found in $CondaInstallationDirectory. Are you sure you want to delete this installation?"
        User-Input
    }
    Else {
        Write-Warning "Removing $CondaInstallationDirectory despite it not containing a esi-conda environment"
    }
}
...
Start-Process (Join-Path -Path $CondaInstallationDirectory -ChildPath "Uninstall-Miniforge3.exe") -ArgumentList "/S" -Wait
Write-Host "All done."
